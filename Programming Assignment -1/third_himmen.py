...
for k in range(3):
    ...
    for j in range(1,N-1):
        ...
        while(himmelblaus(xx,yy)>(himmelblaus(last_xy[0],last_xy[1])-alpha*step_size*np.inner(vect1,vect2))):
...
ax.set_xlabel(r'$x$')
ax.set_ylabel(r'$y$')
titlex='Contour plot for himmelblaus functions with step size(Back_tracking)'
ax.set_title(titlex)
plt.show()
